...
export function loadTracking() {
  try {
    ...
    // Garantir que fases sempre existe
    if (!data.fases) {
      data.fases = {};
    }
    ...
    console.error("[Tracking] Erro ao ler tracking:", e.message);
    // Retorna estrutura padrão se arquivo estiver corrompido
    return {
      project: "INDUSALES v5.0",
      fases: {
        "1.1": { nome: "Autenticação e Hierarquia", items: [], progresso: 0 },
...
export function saveTracking(data) {
  try {
    ...
    console.error("[Tracking] Erro ao salvar:", e.message);
  }
}